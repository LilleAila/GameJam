...
public class GenerateMesh4 : MonoBehaviour
{
    ...
    private static float GetHeight(int x, int z, int xSegments, int zSegments, Vector2 noiseOffset, float noiseScale)
    {
        ...
        if (usePerlinNoise)
            ...
            return TerrainController4.noisePixels[(int)noiseX % TerrainController4.noisePixels.Length][(int)noiseZ % TerrainController4.noisePixels[0].Length];
    }
}
